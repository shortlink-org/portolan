...
Реализовано: Settings классифицирует диагностику в стабильные rules, агрегирует
повторы внутри проекта и trial, сортирует по severity и фильтрует по plugin,
rule, severity и состоянию active/suppressed. Каждая группа показывает следующий
шаг и несколько исходных сообщений. `warningPolicies` в `portolan.json`
использует типизированные CEL-выражения над plugin, rule, severity, project,
phase, ref, message и count. Выражение проверяется при чтении manifest и должно
возвращать bool; suppress требует непустую причину. Решение вычисляется во время
generation и сохраняется в build report, поэтому UI не исполняет политики.
Подавленные сообщения не теряются и доступны отдельным фильтром.

...
// because everything in it fits. These are the other two shapes — an estate
// too wide to fit, and a schema shaped like every mistake a schema can make —
// and they exist so a canvas is asserted against them rather than eyeballed
// against the sample.
//
...
function aggregate(serviceId: string, slug: string, name: string): Aggregate {
...
function service(contextId: string, slug: string, aggregates: string[]): Service {
  ...
  return {
    ...
    provides: [],
    consumes: [],
    aggregates: aggregates.map((a) => aggregate(id, a, a)),
    stores: [`${id}.pg`],
  };
}

/**
 * Thirty stores, two hundred tables, and foreign-key chains six deep.
 *
 * The chains are the point. A layered layout is cheap on a star and expensive
 * on a chain, and a canvas that reads well at four tables can put the sixth
 * layer off the right-hand edge without anyone noticing until an estate this
 * size arrives.
 */
export function wideCatalog(): Catalog {
  ...
  // 6 contexts x 5 services = 30 services, one store each.
  for (let c = 0; c < 6; c += 1) {
    const contextId = `ctx${c}`;
    const services: Service[] = [];
    for (let s = 0; s < 5; s += 1) {
      const svc = service(contextId, `svc${s}`, ["root"]);
      services.push(svc);

      const tables: Table[] = [];
      const storeId = `${svc.id}.pg`;
      // A chain of six, then singletons, to land on 200 tables overall.
      for (let t = 0; t < 7; t += 1) {

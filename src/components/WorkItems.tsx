          ...
          <div className="max-h-[min(32rem,65vh)] divide-y divide-line overflow-y-auto">
            {group.map(({ item, links }) => <section key={item.id} className="p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="ml-auto inline-flex items-center gap-1 text-xs text-muted"><TaskTrackerIcon provider={item.provider} size={13} />{Object.hasOwn(TRACKER_PROVIDERS, item.provider) ? TRACKER_PROVIDERS[item.provider as TaskTracker["provider"]].label : item.provider} · {item.tracker}</span>
              </div>

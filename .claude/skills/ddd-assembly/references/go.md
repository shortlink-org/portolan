...
di/
  ...
  provider/
    ...
    cache.go
    bus.go          in-proc buses bound to Publisher ports (local/test)
    outbox.go       outbox publishers bound to Publisher ports; relay with policy subscriptions
...
Subscribing a policy, in `ProvideRelay`:

```go
userrepo.Handle(relay, map[string]userrepo.Handler{
    userevent.TopicPasswordChanged: revoke.Handle,
})
```

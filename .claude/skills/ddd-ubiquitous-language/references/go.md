...
| Thing | Form | Example |
|---|---|---|
| aggregate package | singular noun | `user/domain`, `session/domain` |
...
| command | the business verb | `ChangePassword`, `Revoke` |
...
| event name | `<context>.<Event>` | `auth.PasswordChanged` |
...
| adapter package | port it serves, under the technology | `session/infrastructure/repository`, `user/infrastructure/bus`, `risk` |
...
Translation at the boundary, `session/infrastructure/risk/client.go`: `riskpb.Verdict_VERDICT_BLOCK`

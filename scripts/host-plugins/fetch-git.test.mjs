...
const options = (remote, cacheDir) => ({ cache: cacheDir, repos: [{ repo: remote.url, commit: remote.commit }] });
...
describe("fetch-git", { timeout: 30_000 }, () => {
  it("writes the whole source tree, a lock and a pin", () => {
    ...
    const response = fetch(options(remote, cache()), online, { generatedAt });
    ...
    expect(lock.repos[0]).toMatchObject({ repo: remote.url, commit: remote.commit });
    expect(lock.repos[0].files.map((file) => file.path)).toEqual(["README.md", "proto/shop/v1/orders.proto", "services/oms/README.md", "services/oms/internal/domain/order/order.go"]);
    expect(lock.repos[0].files[0]).toMatchObject({ size: 7 });
    expect(lock.repos[0].files[0].sha256).toMatch(/^[0-9a-f]{64}$/);
    expect(response.warnings[0].message).toContain("skipped 1 binary file");
  ...
  it("replays the committed copy offline, byte for byte, and says so", () => {
    const remote = repository();
    const cacheDir = cache();
    const first = fetch(options(remote, cacheDir), online);
    write(cacheDir, first);

    const replayed = fetch(options(remote, cacheDir), offline);
    ...
    expect(replayed.warnings).toHaveLength(2);
    expect(replayed.warnings[0].message).toContain("offline");

    // CI verifies what was committed and never opens a socket.
    const inCi = fetch(options(remote, cacheDir), { CI: "true", [OFFLINE_ENV]: "" });
    expect(inCi.warnings).toHaveLength(2);
  });
  ...
  it("does not read a large known binary into the plugin response and sniffs unknown binary files", () => {
    ...
    const response = fetch(options(remote, cache()), online);
    ...
    expect(lock.files.map((file) => file.path)).toEqual(["README.md", "proto/shop/v1/orders.proto", "services/oms/README.md", "services/oms/internal/domain/order/order.go"]);
  ...
  it("names the edited file when a vendored copy no longer matches its lock", () => {
    ...
    expect(() => fetch(options(remote, cacheDir), offline)).toThrow(/order\.go.*edited by hand/);
  });
  ...
  it("falls back to the committed copy when the forge is gone", () => {
    ...
    const fallback = fetch(options(remote, cacheDir), online);
    expect(names(fallback)).toEqual(names(first));
    expect(fallback.warnings).toHaveLength(2);
  ...
  it("resolves an unpinned repository online with a warning, and refuses it offline", () => {
    const remote = repository();
    const opts = { cache: cache(), repos: [{ repo: remote.url, ref: "main" }] };
  ...
  it("reaches a commit through its branch when the remote refuses it by name", () => {
    ...
    const response = fetch(options(remote, cache()), online);
    // All four text files, the lock, and the fragment naming the commit.
  ...
  it("runs through the host like any plugin, warnings kept beside the files", async () => {
    const remote = repository();
    const cacheDir = cache();
    write(cacheDir, fetch(options(remote, cacheDir), online));
    const saved = { CI: process.env.CI, [OFFLINE_ENV]: process.env[OFFLINE_ENV] };
    process.env.CI = "";
    process.env[OFFLINE_ENV] = "1";
    try {
      const result = await runPlugin({ name: "git", host: "fetch-git" }, { portolanVersion: "0.1.0", options: options(remote, cacheDir) });
      expect(result.files.map((file) => file.name)).toHaveLength(6);
      expect(result.warnings).toEqual([`${remote.url}: not fetched (offline); the copy committed in this repository is used unchanged`, `${remote.url}: skipped 1 binary file (10 bytes); metadata is recorded in ${LOCK_NAME}`]);

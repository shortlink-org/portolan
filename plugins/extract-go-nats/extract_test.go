...
func extracted(t *testing.T, root string) (catalog.Catalog, plugin.Response) {
	t.Helper()
	resp, err := extract(plugin.Input{Root: root}, Options{Context: "shop", Service: "pricing"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSourcesAreSpelledFromTheRepository(t *testing.T) {
	for _, tc := range []struct{ root, repository, want string }{
		{"examples/shop/pricing", "", "examples/shop/pricing/app/app.go:5"},
		{"vendor/repos/acme/shop", "vendor/repos/acme/shop", "app/app.go:5"},
	} {
		t.Run(tc.root, func(t *testing.T) {
			t.Chdir(t.TempDir())
			write(t, tc.root, "go.mod", "module example.com/svc\n")
			write(t, tc.root, "app/app.go", "package app\n\nimport \"github.com/nats-io/nats.go\"\n\nfunc A(nc *nats.Conn) { nc.Publish(\"a.first\", nil) }\n")
			...
			if got := channels(out)["a.first"].Source; got != tc.want {

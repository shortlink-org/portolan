...
type flowOptions struct {
	...
	// externals maps a proto package or an api id to the system outside the
	// estate that answers to it, when the manifest names one rather than
	// leaving it to the document beside the client.
...
func (r *flowReader) endpointFlow(endpoint endpointDecl) (catalog.Flow, bool) {
	...
	return catalog.Flow{
		...
		Trigger:      &catalog.FlowTrigger{Kind: endpoint.triggerKind, Label: endpoint.triggerLabel, Confidence: "high"},
...
func (r *flowReader) policyFlowsIn(dir string) []catalog.Flow {
	...
	for _, name := range names {
		...
		out = append(out, catalog.Flow{
			...
			Source:       decl.source,
			Trigger:      &catalog.FlowTrigger{Kind: "event", Label: event.name, Confidence: "high"},
			Owner:        r.opts.context,
			Participants: d.lanes,
			Steps:        d.steps,
		})

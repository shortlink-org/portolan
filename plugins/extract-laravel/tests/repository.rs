...
//! the copy taken off. The inferred OpenAPI document is the exception: the host
//! writes it into this workspace, so its path is spelled from the workspace.

use std::path::Path;

use serde_json::Value;

const ROOT: &str = "testdata/shop";

fn files(repository: &str) -> Vec<(String, String)> {
    let cwd = Path::new(env!("CARGO_MANIFEST_DIR"));
    let request = format!(
        r#"{{"input":{{"root":"{ROOT}","output":"{ROOT}/portolan","repository":"{repository}"}},"options":{{"context":"shop","contextName":"Shop","service":"shop"}}}}"#
    );

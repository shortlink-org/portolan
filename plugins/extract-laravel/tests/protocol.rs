...
#[test]
fn describes_itself() {
    let raw = portolan_extract_laravel::serve(r#"{"kind":"describe"}"#, Path::new(".")).unwrap();
    let response: serde_json::Value = serde_json::from_str(&raw).unwrap();
    assert_eq!(response["describe"]["name"], "extract-laravel");
...
fn every_option_is_described_and_matches_the_struct() {
    ...
    for (name, property) in properties {
        assert!(
            property["description"].as_str().is_some_and(|d| !d.is_empty()),
    ...
    let raw = serde_json::json!({ "input": { "root": "testdata/app" }, "options": all }).to_string();
    portolan_extract_laravel::serve(&raw, Path::new(env!("CARGO_MANIFEST_DIR"))).expect("every declared option is accepted");
    let raw = serde_json::json!({ "input": { "root": "testdata/app" }, "options": { "nope": 1 } }).to_string();
    let err = portolan_extract_laravel::serve(&raw, Path::new(env!("CARGO_MANIFEST_DIR"))).unwrap_err();
    assert!(err.contains("nope"), "{err}");
}

#[test]
fn refuses_to_extract_nothing() {
    let err = portolan_extract_laravel::serve(r#"{"input":{"root":""}}"#, Path::new(".")).unwrap_err();

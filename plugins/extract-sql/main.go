...
type Options struct {
	...
	// Repositories optionally pins where adapters live, relative to the input
	// root. Empty discovers horizontal and feature-sliced layouts.
	Repositories string `json:"repositories,omitempty"`
	// Projectors does the same for projection adapters.
...
func run(stdin io.Reader, stdout io.Writer) error {
	return plugin.Serve(stdin, stdout, descriptor(), func(req plugin.Request, opts Options) (plugin.Response, error) {
		...
		return extract(req.Input, opts), nil
	})
}
...
func extract(in plugin.Input, opts Options) plugin.Response {
	...
	if len(tables) == 0 {
		...
		fragment.Contexts[0].Services[0].Stores = []string{storeID}

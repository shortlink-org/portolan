...
// Where a document keeps its named schemas: under components in OpenAPI 3, and
// under definitions in Swagger 2.0. A reference is a pointer to one of these
// two places, and the name is what follows.
var schemaHomes = []string{"components/schemas/", "definitions/"}
...
// never came back.
func (d *document) shapes(item, operation *yaml.Node) (string, string) {
	request := d.bodyName(child(operation, "requestBody"))
	if request == "" {
		request = d.bodyParameter(operation, item)
	}
...
// several.
//
// OpenAPI 3 puts the schema under a media type; Swagger 2.0 puts it straight on
// the response. Both are read, so a document generated by a tool that still
// writes 2.0 - swag, for one - names its shapes like any other.
func (d *document) bodyName(node *yaml.Node) string {
	...
	if schema := child(body, "schema"); schema != nil {
		return bodyDoc.schemaTypeName(schema)
	}

	return ""
}

// bodyParameter is the Swagger 2.0 request body: a parameter with `in: body`,
// on the operation or inherited from its path item, whose schema is the shape.
// The operation's own list is read first, because it is the one that wins when
// both declare one.
func (d *document) bodyParameter(operation, item *yaml.Node) string {
	for _, owner := range []*yaml.Node{operation, item} {
		for _, param := range itemsOf(child(owner, "parameters")) {
			paramDoc, param := d.follow(param)
			if text(child(param, "in")) != "body" {
				continue
			}
			return paramDoc.schemaTypeName(child(param, "schema"))
		}
	}
...
func schemaName(pointer string) (string, bool) {
	for _, home := range schemaHomes {
		name, ok := strings.CutPrefix(pointer, home)
...
// refName is the name a reference spells even when it points at nothing: a
// document that names a schema it never defines still tells the reader what the
// author called it.

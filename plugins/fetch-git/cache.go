...
//
// The cache is not a second copy of anything: it IS the tree. The files this
// plugin fetched were written by the host through the ordinary Response.Files
// path, so they are committed, reviewable, and removed when the step stops
// naming them. Reading them back and checking them against the lock is what
// lets a build that never opens a socket produce byte-identical output.
...
// cached is one repository's files, read back from the tree.
type cached struct {
	lock  LockRepo
	files map[string][]byte
}

// replay reads a repository's committed copy and checks it against its lock.
//
...
func replay(dir string) (*cached, error) {
	...
	if len(lock.Repos) != 1 {
		return nil, fmt.Errorf("%s names %d repositories; expected exactly one", filepath.ToSlash(at), len(lock.Repos))
	}

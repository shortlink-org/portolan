...
var catalogCoverage = map[reflect.Type]string{
	...
	reflect.TypeOf(catalog.Deployment{}):       "id name project environment cluster namespace repo path chart targetRevision revision tool url service images",
	...
	reflect.TypeOf(catalog.Table{}):            "id name doc columns indexes persists role accesses",
	...
	reflect.TypeOf(catalog.Flow{}):             "id slug name summary source trigger entrypoint includes owner participants steps",
	reflect.TypeOf(catalog.FlowTrigger{}):      "kind label confidence",
	reflect.TypeOf(catalog.Participant{}):      "id kind context label",
	reflect.TypeOf(catalog.Step{}):             "type id from to kind ref label status note line replyTo http continuesAt reaches handoff storeAccess",

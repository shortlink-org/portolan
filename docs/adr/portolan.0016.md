...
1. **Built-in checks stay code with a passport in a file; new rules are CEL
   ...
   deployment, flow, aggregate, call - and `when` and `message` are CEL over a flat, typed
...
A rule of the manifest's own reads one subject - a service, an event, a
channel, a table, a deployment, a flow, an aggregate or a call - as a flat record - strings,

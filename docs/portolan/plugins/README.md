...
rather than guessed channels. A custom topic and an event domain have the same
endpoint shape, so domain endpoints are named explicitly with the `domains`
option. For those endpoints, the Event Grid-schema `Topic` field or CloudEvent
`source` becomes the domain topic and the channel address is `domain/topic`.
...
complete grammar, stable-id guidance, canonical participant references, store
access syntax, failure behaviour, and an executable golden example live in the
focused `docs/flows-authoring.md` guide. Keeping that
contract beside its test fixture prevents this plugin protocol overview from
becoming a second, stale language reference.

...
3. Verifies the stored hash through its password-verifier port, and tells the
   lockout how it went: a wrong one counts, and the fifth in a row locks the
   account for fifteen minutes; a right one clears the count.

...
func outboxSetup(t *testing.T) *outboxHarness {
	...
	store, _, unit := testStoreWithDB(t,
		migrationSource{FS: repo.Migrations, Name: repo.Name},
...
func TestPublishOutsideAUnitOfWorkIsRefused(t *testing.T) {
	...
	err := h.publisher.Publish(context.Background(),
		[]userdomainevent.Event{userdomainevent.NewUserRegistered("u1", "ada@example.com", now)})
...
func TestRollbackTakesTheEvent(t *testing.T) {
	...
	err := h.uow.Do(ctx, func(ctx context.Context) error {
		if err := h.publisher.Publish(ctx, []userdomainevent.Event{
			userdomainevent.NewUserRegistered("u1", "ada@example.com", now),
		}); err != nil {
			return err
		}
		return boom
...
func TestRoundTrip(t *testing.T) {
	...
	err := h.uow.Do(ctx, func(ctx context.Context) error {
		return h.publisher.Publish(ctx, []userdomainevent.Event{
			userdomainevent.NewPasswordChanged("u1", "s1", now),
		})
	})
	if err != nil {
		t.Fatal(err)

...
import (
	...
	domain "github.com/shortlink-org/portolan/examples/auth/internal/user/domain"
	repo "github.com/shortlink-org/portolan/examples/auth/internal/user/infrastructure/repository"
)
...
func store(t *testing.T) *repo.Postgres {
	...
	router, unit := testStore(t, migrationSource{FS: repo.Migrations, Name: repo.Name})
...
func newUser(t *testing.T, id, mail string) *domain.User {
	t.Helper()
	u, _, err := domain.Register(id, mail, mustHash(t, plaintext), now)
	if err != nil {
		t.Fatal(err)
	}
	return u
...
func TestSaveAndRead(t *testing.T) {
	...
	if !passwordMatches(byID.Password, plaintext) {
...
func TestStaleWriteIsRefused(t *testing.T) {
	...
	if _, err := first.ChangePassword(mustHash(t, "NewPassw0rd"), "s1", now); err != nil {
		t.Fatal(err)
	}
...
func TestChangesLandOnlyThroughSave(t *testing.T) {
	...
	if _, err := loaded.ChangePassword(mustHash(t, "NewPassw0rd"), "s1", now); err != nil {
		t.Fatal(err)
	}

	before, _ := s.ByID(ctx, "u1")
	if !passwordMatches(before.Password, plaintext) {
	...
	if !passwordMatches(after.Password, "NewPassw0rd") {

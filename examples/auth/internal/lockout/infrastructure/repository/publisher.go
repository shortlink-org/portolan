...
// Publisher writes the lockout domain's events into the transaction in flight.
//
// It sits next to the aggregate's repository because both are adapters for the
// same domain's ports: one puts a Lockout into rows, the other puts what happened
...
var _ lockout.Publisher = (*Publisher)(nil)
...
func (p *Publisher) Publish(ctx context.Context, events []domainevent.Event) error {
	messages := make([]*message.Message, 0, len(events))

	for _, e := range events {
		msg, err := integrationevent.Marshal(e)
		if err != nil {
			return err
		}
		...
		_, span := messaging.StartPublish(ctx, integrationevent.Topic, msg, e.Name())
		span.End()
		messages = append(messages, msg)
	}

	return p.outbox.Publish(ctx, integrationevent.Topic, messages...)
}

// Bus is what the relay hands an integration event to once it is back out of
// the outbox. The domain event has already been translated to its stable wire
// contract; policies subscribe to that contract, never to aggregate types.
type Bus interface {
	Publish(context.Context, []integrationevent.Event) error
...
func Handle(relay *sdkoutbox.Relay, bus Bus) error {
	return relay.Handle(integrationevent.Topic, func(ctx context.Context, msg *message.Message) (err error) {
		name := msg.Metadata.Get(messaging.MetadataEventName)

		ctx, span := messaging.StartRelay(ctx, bus.System(), integrationevent.Topic, name)
		defer func() { messaging.EndWith(span, err) }()

		e, err := integrationevent.Unmarshal(msg)
		if err != nil {
			return fmt.Errorf("lockout: %s: %w", name, err)
		}
		if e == nil {
		...
		return bus.Publish(ctx, []integrationevent.Event{e})
	})
}

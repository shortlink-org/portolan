...
func TestCachedThroughARealRedis(t *testing.T) {
	...
	repository := repo.NewCached(store.mock, testRedis(t), cacheTTL, func() time.Time { return now })
...
func TestCachedKeepsNothingItCannotStore(t *testing.T) {
	...
	store := newMockStore(t, expired)
	memory := testRedis(t)
	repository := repo.NewCached(store.mock, memory, cacheTTL, func() time.Time { return now })

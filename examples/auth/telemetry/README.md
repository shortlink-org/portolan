...
| `publish auth.PasswordChanged`, kind producer | `messaging.StartPublish`, in the repository publishers, with `event.name` |
| `consume auth.PasswordChanged`, kind consumer | `messaging.StartConsume`, in the dispatcher, with `event.name` |

The consumer span is a child of the producer's, not a trace of its own: the
publisher puts its span context on the message, the outbox keeps it in the
row, and the relay's middleware reads it back. That one line is what lets a
reader of the trace, and of the catalog, follow a password change through the
table and into the policy that ends the sessions.

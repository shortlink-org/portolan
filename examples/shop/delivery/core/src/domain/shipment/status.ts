...
// quietly. It starts waiting for the money: nothing leaves the warehouse
// before the ledger says the payment was captured (ADR core.0002). Nothing
// leads out of `delivered` or `lost`: both are the end, and a parcel that turns
// up after being written off is a new shipment.

export type ShipmentStatus = "awaiting-payment" | "planned" | "dispatched" | "in-transit" | "delivered" | "lost";

/** Where a shipment can go from where it is. */
export const TRANSITIONS: Readonly<Record<ShipmentStatus, readonly ShipmentStatus[]>> = {
  "awaiting-payment": ["planned", "lost"],
